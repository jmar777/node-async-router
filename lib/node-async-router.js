...
function wrapArg(isParamHandler, fn) {
    ...
    // make sure that all requests to proxy object properties goes directly to fn object
    var proxyHandler = {
    ...
    if (fn.length === 4 && !isParamHandler) {
        return new Proxy(function(err, req, res, next) {
            var ret;
    ...
    return new Proxy(function(req, res, next) {
        var ret;
